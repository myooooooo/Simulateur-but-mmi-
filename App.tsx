...
import { RotateCcw, Award, AlertCircle, ChevronRight, Calculator, Menu, X, Download, Upload, GraduationCap, Terminal, Palette, Presentation, Wand2, User, Power, Home, FileJson, FileText, Sparkles, AlertTriangle, Printer } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
               <div className="w-1 h-3 rounded bg-slate-300"></div> Ressources
            </h4>
    ...
    <div className="flex flex-col h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <div className="flex-1 flex flex-col md:flex-row h-full overflow-hidden">
          ...
          <div className="p-4 border-t border-slate-800 space-y-3">
            <div className="grid grid-cols-2 gap-2">
               <button onClick={() => window.print()} className="flex flex-col items-center justify-center p-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-[10px] text-slate-300 transition-colors">
                  <Printer className="w-4 h-4 mb-1" /> PDF
               </button>
               <button onClick={() => {
                 ...
                 a.href = url;
                 a.download = `notes_zineb_${activeSemesterId}.json`;
                 a.click();
        ...
        <main className="flex-1 flex flex-col h-full overflow-hidden bg-slate-50">
          <header className="bg-white border-b border-slate-200 h-20 flex items-center justify-between px-4 md:px-8 flex-shrink-0 z-10 print:bg-white print:shadow-none">
            <div>
            ...
            <div className={`px-5 py-2 rounded-xl border-2 flex items-center gap-4 transition-all duration-500 ${isValidated ? 'bg-emerald-50 border-emerald-200 shadow-sm shadow-emerald-100' : 'bg-slate-50 border-slate-100'}`}>
               <div className="text-right">
                 <div className="text-[10px] text-slate-500 uppercase font-black tracking-tighter">Moyenne Générale</div>
                 <div className={`text-3xl font-black leading-none ${isValidated ? 'text-emerald-600' : 'text-slate-800'}`}>{globalAverage}</div>
               </div>
               <div className="h-10 w-10 flex items-center justify-center bg-white rounded-full shadow-inner border border-slate-100">
            ...
            <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-6">
                {failedCompetencies.length > 0 && (
                  <div className="p-4 bg-rose-50 border border-rose-200 rounded-xl flex items-center gap-3 text-rose-800 text-sm font-bold shadow-sm animate-pulse">
                    <AlertCircle className="w-5 h-5 text-rose-500" />
                    Moyenne d'UE inférieure à 8/20 : Semestre non validable.
                  </div>
                )}
                ...
                <div className="bg-slate-900 text-white p-6 rounded-xl shadow-xl">
                   ...
                   <div className="space-y-4">
                      <div className="flex justify-between items-center">
                         <span className="text-sm text-slate-300">Global &ge; 10</span>
                         {globalAverage >= 10 ? <Award className="w-4 h-4 text-emerald-400" /> : <X className="w-4 h-4 text-slate-600" />}
                      </div>
                      <div className="flex justify-between items-center">
                         <span className="text-sm text-slate-300">UEs &ge; 8</span>
                         {failedCompetencies.length === 0 ? <Award className="w-4 h-4 text-emerald-400" /> : <X className="w-4 h-4 text-rose-500" />}
                      </div>
                      <div className={`mt-4 pt-4 border-t border-slate-800 text-center font-black uppercase text-xl ${isValidated ? 'text-emerald-400' : 'text-slate-600'}`}>
                         {isValidated ? 'ADMIS' : 'AJOURNÉ'}
                      </div>
                   </div>
                </div>
              </div>
